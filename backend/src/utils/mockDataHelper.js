/**
 * Mock Data Helper Utility
 * Provides on-the-fly mock data generation and fallback capabilities
 */
...
/**
 * Generate a mock ID (MongoDB ObjectId)
 */
...
const generateMockUsers = () => {
  if (mockDataCache.users.length > 0) return mockDataCache.users;
...
const generateMockVillages = () => {
  if (mockDataCache.villages.length > 0) return mockDataCache.villages;
  ...
  mockDataCache.villages = villages;
  return villages;
};

/**
 * Generate mock springs for villages
 */
const generateMockSprings = (villages = null) => {
  if (mockDataCache.springs.length > 0) return mockDataCache.springs;

  const mockVillages = villages || generateMockVillages();
  ...
  mockVillages.forEach((village, vIdx) => {
 ...
 */
const generateMockRainfallData = (villages = null) => {
  if (mockDataCache.rainfallData.length > 0) return mockDataCache.rainfallData;
  ...
  mockVillages.forEach(village => {
    ...
    for (let day = 0; day < 30; day++) {
      ...
      rainfallRecords.push({
        ...
        longitude: lon,
        recorded_date: date,
        rainfall_mm: Math.max(0, Math.random() * 40 - 5),
  ...
  mockDataCache.rainfallData = rainfallRecords;
 ...
 */
const generateMockElevationData = (villages = null) => {
  if (mockDataCache.elevationData.length > 0) return mockDataCache.elevationData;
  ...
  mockDataCache.elevationData = elevationRecords;
  return elevationRecords;
};

/**
 * Generate mock recharge analysis
 */
const generateMockRechargeAnalysis = (springs = null, villages = null) => {
  if (mockDataCache.rechargeAnalysis.length > 0) return mockDataCache.rechargeAnalysis;

  const mockSprings = springs || generateMockSprings();
  const mockVillages = villages || generateMockVillages();
  const users = generateMockUsers();
  ...
  mockSprings.forEach(spring => {
    const village = mockVillages.find(v => v._id === spring.village_id);
  ...
  mockDataCache.rechargeAnalysis = analyses;
  return analyses;
};

/**
 * Generate mock field verifications
 */
const generateMockFieldVerifications = (springs = null, villages = null) => {
  if (mockDataCache.fieldVerifications.length > 0) return mockDataCache.fieldVerifications;
  ...
  mockSprings.forEach(spring => {
    ...
    for (let i = 0; i < surveyCount; i++) {
      ...
      const survey = {
        ...
        notes: `Field survey conducted. Spring appears ${status === 'verified' ? 'healthy' : 'needs monitoring'}.`,
  ...
  mockDataCache.fieldVerifications = verifications;
  return verifications;
};

/**
 * Generate mock uploaded photos
 */
const generateMockUploadedPhotos = (verifications = null) => {
  if (mockDataCache.uploadedPhotos.length > 0) return mockDataCache.uploadedPhotos;

  const mockVerifications = verifications || generateMockFieldVerifications();
  const photos = [];
  ...
  mockDataCache.uploadedPhotos = photos;
  return photos;
};
...
const initializeMockData = () => {
  if (cacheInitialized) return mockDataCache;

  generateMockUsers();
  const villages = generateMockVillages();
  const springs = generateMockSprings(villages);
  generateMockRainfallData(villages);
  generateMockElevationData(villages);
  generateMockRechargeAnalysis(springs, villages);
  generateMockFieldVerifications(springs, villages);
  generateMockUploadedPhotos();
...
/**
 * Get specific mock data or entire cache
 */
const getMockData = (type = null) => {
  initializeMockData();
  
  if (type) {
    return mockDataCache[type] || [];
  }
  return mockDataCache;
};
...
module.exports = {
  ...
  generateMockElevationData,
  generateMockRechargeAnalysis,
  generateMockFieldVerifications,
  generateMockUploadedPhotos,
  initializeMockData,
  clearMockDataCache,
  getMockData
